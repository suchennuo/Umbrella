...
//    }
    ...
    private void addMenu(Context context) {
        if (menuOperator != null) {
            ...
            menuHeight = DensityUtil.dip2px(getContext(), 30); //设置 menu 宽度和高度
        }
...
//        stopClipRunner();
    ...
    /**
     * 利用 scroller.startScroll(int startX, int startY, int dx, int dy, int duration);
     * 得到随着时间，不断变化的 x 值
     */
    private class ClipRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (menu != null && menu.getVisibility() == VISIBLE) {
                if (mClipScroller.computeScrollOffset()) { // true 滚动完成
                    menu.setOffset(currentX); //不断设置 offset 进行绘制
...
//            com.miui.securitycenter/com.miui.permcenter.permissions.PermissionsEditorActivity
            ...
            Log.d("FloatBall", "type toast");
        } else {
            mLayoutParams.type = WindowManager.LayoutParams.TYPE_PHONE; //TYPE_PHONE;
        }
